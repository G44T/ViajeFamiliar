/**
 * useTripStore — versión Firebase con sincronización en tiempo real.
 */
...
const COLORS = [
  '#c8502a','#2a6fa8','#3a7a52','#d4a017',
  '#7a3a8a','#2a8a8a','#a85028','#5a7a2a',
];
...
export function useTripStore() {
  ...
  useEffect(() => {
    ...
    setError(null);
    const unsubs = [
      svc.subscribeTrip(tripId,
        (data) => { setTrip(data); setLoading(false); },
  ...
  const removeTraveler = useCallback(async (travelerId) => {
    if (!tripId) return;
    await svc.removeTraveler(tripId, travelerId);
  }, [tripId]);

  const addExpense = useCallback(async (expense) => {
    if (!tripId) return;
    await svc.addExpense(tripId, expense);
  }, [tripId]);

  const removeExpense = useCallback(async (expenseId) => {
    if (!tripId) return;
    await svc.removeExpense(tripId, expenseId);
  }, [tripId]);

  const updateExpense = useCallback(async (expenseId, data) => {
    if (!tripId) return;
    await svc.updateExpense(tripId, expenseId, data);
  }, [tripId]);

  return {
    state: { trip, travelers, expenses },
    tripId,
    loading,
    error,
    createTrip,
