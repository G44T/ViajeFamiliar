...
export default function SetupScreen({ onSetup, onJoin, error }) {
  ...
  const [tripData, setTripData] = useState({
    name: '', destination: '', currency: 'PEN', startDate: '', endDate: '',
  });
  ...
  const handleTripNext = (e) => {
    e.preventDefault();
    if (!tripData.name.trim() || !tripData.destination.trim()) return;
    setStep(2);
  };

  const addTraveler = () => {
    if (!newName.trim() || travelers.length >= 8) return;
    setTravelers(p => [...p, newName.trim()]);
    setNewName('');
  };

  const handleFinish = async () => {
    if (travelers.length < 2 || submitting) return;
    setSubmitting(true);
    await onSetup(tripData, travelers);
  };

  const handleJoin = async (e) => {
    e.preventDefault();
    if (!joinId.trim() || submitting) return;
    setSubmitting(true);
    onJoin(joinId.trim());
  };
      ...
      <div className={styles.container}>
        ...
        <div className={styles.card}>
          ...
          <div className={styles.modeTabs}>
            <button
              className={`${styles.modeTab} ${mode === 'new' ? styles.modeActive : ''}`}
              onClick={() => { setMode('new'); setStep(1); }}
            >
              ✈️ Nuevo viaje
            </button>
            <button
              className={`${styles.modeTab} ${mode === 'join' ? styles.modeActive : ''}`}
          ...
          {mode === 'join' && (
            <form onSubmit={handleJoin} className={`${styles.form} animate-fade`}>
              <p className={styles.joinHint}>
                Pide a quien creó el viaje que te comparta el <strong>ID del viaje</strong> y pégalo aquí.
              </p>
              <div className={styles.field}>
                <label>ID del viaje</label>
                <div className={styles.inputWrap}>
                  <Link size={16} className={styles.icon} />
                  <input
                    type="text"
            ...
            <>
              <div className={styles.steps}>
                <div className={`${styles.step} ${step >= 1 ? styles.active : ''}`}>
                  <div className={styles.stepDot}>1</div><span>El viaje</span>
                </div>
                <div className={styles.stepLine} />
                <div className={`${styles.step} ${step >= 2 ? styles.active : ''}`}>
                  <div className={styles.stepDot}>2</div><span>Viajeros</span>
                </div>
              </div>

              {step === 1 && (
                <form onSubmit={handleTripNext} className={`${styles.form} animate-fade`}>
                  <div className={styles.field}>
                    <label>Nombre del viaje</label>
                    <div className={styles.inputWrap}>
                      <Plane size={16} className={styles.icon} />
                      <input type="text" placeholder="Ej: Vacaciones en Cusco 2025"
                        value={tripData.name} onChange={e => setTripData(p => ({...p, name: e.target.value}))} required />
                    </div>
                  </div>
                  <div className={styles.field}>
                    <label>Destino</label>
                    <div className={styles.inputWrap}>
                      ...
                      <input type="text" placeholder="Ej: Cusco, Perú"
                        value={tripData.destination} onChange={e => setTripData(p => ({...p, destination: e.target.value}))} required />
                    </div>
                  </div>
                  <div className={styles.row2}>
                    <div className={styles.field}>
                      ...
                      <div className={styles.inputWrap}>
                        ...
                        <input type="date" value={tripData.startDate}
                    ...
                    <div className={styles.field}>
                      ...
                      <div className={styles.inputWrap}>
                        <Calendar size={16} className={styles.icon} />
                        <input type="date" value={tripData.endDate}
                          onChange={e => setTripData(p => ({...p, endDate: e.target.value}))} />
                      </div>
                    </div>
                  </div>
                  <div className={styles.field}>
              ...
              {step === 2 && (
                <div className={`${styles.form} animate-fade`}>
                  <div className={styles.travelerHint}>
                    <Users size={16} />
                    <span>Agrega al menos 2 personas ({travelers.length}/8)</span>
                  </div>
                  <div className={styles.field}>
                    ...
                    <div className={styles.addRow}>
                      <div className={styles.inputWrap} style={{flex:1}}>
                        ...
                        <input type="text" placeholder="Ej: Mamá, Papá, Juan..."
                          value={newName} onChange={e => setNewName(e.target.value)}
                          onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addTraveler())}
                          maxLength={20} />
                      </div>
                      <button type="button" onClick={addTraveler} className={styles.btnAdd}
                        disabled={!newName.trim() || travelers.length >= 8}>+</button>
                    </div>
                  </div>
                  {travelers.length > 0 && (
                    <div className={styles.travelerList}>
                      {travelers.map((name, i) => (
                        <div key={i} className={`${styles.travelerChip} animate-scale`}>
                          <div className={styles.chipAvatar} style={{background: COLORS[i % COLORS.length]}}>
                  ...
                  <div className={styles.btnRow}>
                    ...
                    <button type="button" onClick={handleFinish} className={styles.btnPrimary}
                      disabled={travelers.length < 2 || submitting}>
