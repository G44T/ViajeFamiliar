    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.addCard}>
        ...
        <form onSubmit={handleAdd} className={styles.addForm}>
          <div className={styles.inputWrap}>
            ...
            <input
              type="text"
              placeholder="Nombre del viajero (máx. 8)"
              value={newName}
          ...
          <button
            ...
            disabled={!newName.trim() || travelers.length >= 8}
          >
            ...
            <div key={t.id} className={`${styles.travelerCard} animate-fade`}>
              <div className={styles.travelerLeft}>
                ...
                <div>
                  ...
                  <div className={styles.meta}>
                    {expCount > 0
                      ? `Pagó ${expCount} gasto${expCount !== 1 ? 's' : ''}`
              ...
              <div className={styles.travelerRight}>
                ...
                <button
                  ...
                  onClick={() => {
                    if (!canRemove) {
                      alert('No puedes eliminar a este viajero porque tiene gastos asociados. Elimina primero sus gastos.');
                      return;
                    }
                    if (window.confirm(`¿Eliminar a ${t.name} del viaje?`)) {
                      onRemove(t.id);
                    }
                  }}
                  title={canRemove ? 'Eliminar' : 'Tiene gastos asociados'}
                >
                  <Trash2 size={14} />
                </button>
              </div>
      ...
      {travelers.length === 0 && (
        <div className={styles.empty}>
          <span>👥</span>
          <p>No hay viajeros. Agrega al menos 2.</p>
        </div>
      )}
    </div>
  );
