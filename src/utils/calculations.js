  ...
  expenses.forEach(exp => {
    ...
    if (splitMode === 'custom' && customShares) {
      Object.entries(customShares).forEach(([pid, share]) => {
        if (balances[pid] !== undefined) balances[pid] -= Number(share);
      });
    } else {
      if (!splitAmong || !splitAmong.length) return;
      const share = amount / splitAmong.length;
      splitAmong.forEach(pid => { if (balances[pid] !== undefined) balances[pid] -= share; });
    }
  });
  const settlements = [];
  const debtors = Object.entries(balances).filter(([,v]) => v < -0.01).map(([id,v]) => ({ id, amount: -v })).sort((a,b) => b.amount - a.amount);
...
export function getTravelerById(travelers, id) { return travelers.find(t => t.id === id); }
